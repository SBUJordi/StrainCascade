...
# StrainCascade_assembly_evaluation2.sh - Version 1.0.0
...
# Find all assembly files in genome_assembly_main_abs (except previously chosen best_ev2 assemblies by this script or assemblies generated further downsteam previously (eg. best_ev2, "*_circularised.fasta") 
assemblies=($(find "$genome_assembly_main_abs" -type f -name "*.fasta" ! -name "*_best_ev2*" ! -name "*_best_ev3*" ! -name "*_circularised.fasta"))
...
if [ -n "$best_assembly" ]; then
  echo "Best Assembly (ev2): $best_assembly"
  log "$logs_dir" "StrainCascade_assembly_evaluation.log" "Best Assembly (ev2): $best_assembly"
else
...
do
  if [ -f "$assembly" ]; then
    ...
    file=$(basename "$assembly")
    prefix=${file%.*}
    cp "$assembly" "${assembly_evaluation_output_dir}/${prefix}_best_ev2.fasta"
    
    # Copy the assembly file to the genome_assembly_main_abs with a new name
    cp "$assembly" "${genome_assembly_main_abs}/${prefix}_best_ev2.fasta"
