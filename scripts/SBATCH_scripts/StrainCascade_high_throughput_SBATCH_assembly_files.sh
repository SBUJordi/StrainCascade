...
# StrainCascade_high_throughput_SBATCH_assembly_files.sh
# A script for batch-submitting StrainCascade sbatch jobs for assembly data processing
...
straincascade -a "$input_file" -r main -t \$SLURM_CPUS_PER_TASK -s "$SEQUENCING_TYPE" -o "$RUNS_DIR"
EOF
)
