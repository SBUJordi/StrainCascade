...
# StrainCascade_LJA_assembly.sh - Version 1.0.0
...
lja_output_dir="$output_dir/LJA_assembly_results"
create_directory "$lja_output_dir"  # Ensure the output directory is created

## Start with the actual processing ##
# Check if the file informed_genome_size_estimation.txt is in the genome_assembly_main_abs directory (effectively it will initiate genome size estimation if this is the first genome assembly module)
...
# Run the La Jolla Assembler
log "$logs_dir" "LJA_assembly.log" "Running La Jolla Assembler for $input_file in $lja_output_dir"
apptainer exec \
    --bind "$(dirname "$input_file")":/mnt/input \
    --bind "$lja_output_dir":/mnt/output \
    "$straincascade_genome_assembly" lja \
    -o /mnt/output \
    --reads /mnt/input/$(basename "$input_file") \
    --threads "$threads"
    
# Rename assembly.fasta files in the output directory
cd "$lja_output_dir" || { log "$logs_dir" "LJA_assembly.log" "Error changing directory to $lja_output_dir."; exit 1; }
for assembly_file in assembly.fasta; do
  # Get the prefix from the directory name (remove the "output_" part)
  prefix="${sample_name}_assembly_lja"  # Use the provided sample name
  new_name="${prefix}.fasta"
  mv "$assembly_file" "$new_name"
  log "$logs_dir" "LJA_assembly.log" "Renamed $assembly_file to $new_name"

  # Copy the renamed assembly file to genome_assembly_main_abs
  cp "$new_name" "${genome_assembly_main_abs}/$new_name"
  log "$logs_dir" "LJA_assembly.log" "Copied $new_name to $genome_assembly_main_abs"
done
