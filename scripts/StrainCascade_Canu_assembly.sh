...
# Clean up existing Canu output directory to prevent reuse issues
if [[ -d "$CANU_OUTPUT_DIR" ]]; then
    log "$LOGS_DIR" "$LOG_NAME" "Removing existing Canu output directory to ensure clean run"
    rm -rf "$CANU_OUTPUT_DIR"
fi
